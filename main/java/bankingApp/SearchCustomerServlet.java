...
public class SearchCustomerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql = "SELECT name, address, mobile_no, email, acc_type, dob, id_proof FROM customer WHERE acc_no = ?";
			...
			if (rs.next()) {
				...
				request.setAttribute("customerDetails",
						new String[] { name, address, mobileNo, email, accountType, dob, idProof });
			} else {
		...
		request.getRequestDispatcher("custDetails.jsp").forward(request, response);
	}
}
