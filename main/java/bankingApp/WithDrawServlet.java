...
public class WithDrawServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		double withdrawlAmount = Double.parseDouble(request.getParameter("bill"));
		...
		try {
			...
			if (rs.next()) {
				...
				// Check if withdrawal amount is greater than existing balance
				if (newBalance < 0) {
					...
					out1.println("  text: 'Withdrawal unsuccessful, Insufficient balance ',");
					out1.println("  icon: 'error'");
					out1.println("}).then(() => { window.location.href='dashboard-withdraw.jsp'; });");
					out1.println("</script>");
					out1.println("</body>");
					out1.println("</html>");
					out1.close();
				...
				stmt.setString(1, accNo);
				stmt.setString(2, String.valueOf(withdrawlAmount));
				...
				if (rowsAffected > 0) {
					...
					session.setAttribute("transactionAmount", withdrawlAmount);
					response.setContentType("text/html");
					PrintWriter out1 = response.getWriter();
					out1.println("<!DOCTYPE html>");
					out1.println("<html>");
					...
					out1.println("  text: 'Withdrawal Successful ',");
					...
					// Display error message for unsuccessful update
					response.setContentType("text/html");
					PrintWriter out1 = response.getWriter();
					out1.println("<!DOCTYPE html>");
					out1.println("<html>");
					...
					out1.println("Swal.fire({");
					out1.println("  title: 'Error',");
					out1.println("  text: 'Withdrawal unsuccessful, please try again ',");
					out1.println("  icon: 'error'");
					out1.println("}).then(() => { window.location.href='dashboard-withdraw.jsp'; });");
					out1.println("</script>");
					out1.println("</body>");
